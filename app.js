  ...
  return `
    <article class="post-card">
      <div class="post-header">
        ...
        ${canDelete ? `<button class="delete-btn" onclick="deletePost('${post.id}')">Delete</button>` : ""}
      </div>
...
document.addEventListener("click", function (event) {
  const story = event.target.closest(".story-click");

  if (!story) return;

  const storyName = story.dataset.name;
  const storyImage = story.dataset.image;

  const storyModal = document.getElementById("storyModal");
  const storyModalImg = document.getElementById("storyModalImg");
  const storyHeaderImg = document.getElementById("storyHeaderImg");
  const storyModalName = document.getElementById("storyModalName");

  if (!storyModal || !storyModalImg || !storyHeaderImg || !storyModalName) return;

  storyModalImg.src = storyImage;
  storyHeaderImg.src = storyImage;
  storyModalName.textContent = storyName;

  storyModal.classList.remove("hidden");
});

document.addEventListener("click", function (event) {
  const closeButton = event.target.closest("#closeStoryBtn");
  const modalBackground = event.target.id === "storyModal";

  if (!closeButton && !modalBackground) return;
  ...
  if (storyModalImg) storyModalImg.src = "";
  if (storyHeaderImg) storyHeaderImg.src = "";
});
